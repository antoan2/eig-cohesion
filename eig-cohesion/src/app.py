...
title = html.Div(dbc.Row([dbc.Col(html.H1('EIG Cohesion'), className='m-5', align='center')]))
...
def display_output(values):
    ...
    if ctx.triggered[0]["prop_id"] != '.':
        ...
        print(meeting_hash, value)
        if value == ['done']:
            ru.set_meeting_done(meeting_hash=meeting_hash, weekly_meetings_key= WEEKLY_MEETINGS_IDS, redis_server=REDIS_SERVER)
        else:
            ru.set_meeting_not_done(meeting_hash=meeting_hash, weekly_meetings_key= WEEKLY_MEETINGS_IDS, redis_server=REDIS_SERVER)
    weekly_meetings = ru.load_weekly_meetings(WEEKLY_MEETINGS_IDS, PROMO, REDIS_SERVER)
    weekly_meetings_card = get_weekly_meetings_card(weekly_meetings)
...
#     ctx = dash.callback_context
#     logger = logging.getLogger('eig-cohesion')
# 
#     if not ctx.triggered:
#         button_id = 'No clicks yet'
#     else:
#         button_id = ctx.triggered[0]['prop_id'].split('.')[0]
#         logger.error(ctx.triggered)
#         logger.error(button_id)
# 
#     for meeting in WEEKLY_MEETINGS["meetings"]:
#         if meeting in value:
#             WEEKLY_MEETINGS["meetings"][meeting] = True
#         else:
#             WEEKLY_MEETINGS["meetings"][meeting] = False
#     progress = get_weekly_meeting_progress(WEEKLY_MEETINGS)
#     color = 'danger'
#     if 25 < progress < 75:
#         color = 'warning'
#     elif progress >= 75:
